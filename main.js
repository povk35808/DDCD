...
async function submitReturnDate(sheetName, rowIndex, studentName, studentEmail, returnDate) {
    try {
        ...
        if (result.status === 'success') {
            ...
            if (sheetName === 'ច្បាប់ទៅផ្ទះ') {
                showForm('home-leave');
            } else {
                showForm('entry-leave');
            }
        } else {
            showMessage(`មានបញ្ហាពេលបញ្ជូន: ${result.message}`, false);
        }
    } catch (error) {
        console.error("Error submitting return date:", error);
        showMessage("មានបញ្ហាពេលបញ្ជូនថ្ងៃត្រឡប់មកវិញ។", false);
    } finally {
        homeLeaveSubmitBtn.disabled = false;
        entryLeaveSubmitBtn.disabled = false;
    }
}

// Event listener for the "ច្បាប់ទៅផ្ទះ" dropdown
studentNameDropdownHome.addEventListener('change', (event) => {
    const selectedIndex = event.target.value;
    if (selectedIndex !== "") {
        const selectedRecord = homeLeaveRecords[selectedIndex];
        collegeHomeInput.value = selectedRecord.college;
        leaveDateHomeInput.value = selectedRecord.leaveDate;
        reasonHomeInput.value = selectedRecord.reason;
        homeLeaveSubmitBtn.disabled = false;
    } else {
        homeLeaveSubmitBtn.disabled = true;
    }
...
// Event listener for the "ច្បាប់ចេញចូល" dropdown
studentNameDropdownEntry.addEventListener('change', (event) => {
    const selectedIndex = event.target.value;
    if (selectedIndex !== "") {
        const selectedRecord = entryLeaveRecords[selectedIndex];
        collegeEntryInput.value = selectedRecord.college;
        leaveDateEntryInput.value = selectedRecord.leaveDate;
        reasonEntryInput.value = selectedRecord.reason;
        entryLeaveSubmitBtn.disabled = false;
    } else {
        entryLeaveSubmitBtn.disabled = true;
    }
...
// Event listener for the main login form submission
studentForm.addEventListener('submit', async function (event) {
    event.preventDefault();

    submitBtn.disabled = true;
    submitBtn.textContent = 'កំពុងបញ្ជូន';
    showMessage("កំពុងផ្ទៀងផ្ទាត់ទិន្នន័យ...", true);
    ...
    try {
        ...
        if (result.status === 'success') {
            showMessage("កំពុងទាញយកទិន្នន័យនិស្សិត...", true);
            currentStudentEmail = studentEmailInput.value;
            currentStudentName = studentNameInput.value;
            const studentRecords = await fetchStudentRecords(currentStudentName, currentStudentEmail);
            ...
            userEmailDisplay.textContent = currentStudentEmail;

            showForm('home-leave');
        } else {
            showMessage(result.message, false);
            submitBtn.disabled = false;
            submitBtn.textContent = 'បញ្ជូនទិន្នន័យ';
        }
    } catch (error) {
        console.error('Submission error:', error);
        showMessage('មានបញ្ហាពេលបញ្ជូនទិន្នន័យ សូមព្យាយាមម្តងទៀត។', false);
        submitBtn.disabled = false;
        submitBtn.textContent = 'បញ្ជូនទិន្នន័យ';
    }
});
...
tabHome.addEventListener('click', () => showForm('home-leave'));
tabEntry.addEventListener('click', () => showForm('entry-leave'));

// Event listeners for the new dashboard submit buttons
homeLeaveForm.addEventListener('submit', async function (event) {
    event.preventDefault();
    const selectedIndex = studentNameDropdownHome.value;
    if (selectedIndex !== "") {
        const record = homeLeaveRecords[selectedIndex];
        const studentName = record.name;
        const rowIndex = record.rowIndex;
        const sheetName = 'ច្បាប់ទៅផ្ទះ';
        const returnDate = returnDateHomeInput.value || new Date().toISOString().slice(0, 10);
        homeLeaveSubmitBtn.disabled = true;
        await submitReturnDate(sheetName, rowIndex, studentName, currentStudentEmail, returnDate);
    }
});

entryLeaveForm.addEventListener('submit', async function (event) {
    event.preventDefault();
    const selectedIndex = studentNameDropdownEntry.value;
    if (selectedIndex !== "") {
        const record = entryLeaveRecords[selectedIndex];
        const studentName = record.name;
        const rowIndex = record.rowIndex;
        const sheetName = 'ច្បាប់ចេញចូល';
        const returnDate = returnDateEntryInput.value || new Date().toISOString().slice(0, 10);
        entryLeaveSubmitBtn.disabled = true;
        await submitReturnDate(sheetName, rowIndex, studentName, currentStudentEmail, returnDate);
    }
});
...
logoutBtn.addEventListener('click', () => {
    ...
    checkLocationAndEnableForm();
});

// Custom Alert Listeners
closeWelcomeAlertBtn.addEventListener('click', () => {
    welcomeAlert.classList.remove('visible');
});

closeSuccessAlertBtn.addEventListener('click', () => {
    successAlert.classList.remove('visible');
});
...
window.onload = () => {
    ...
    welcomeAlert.classList.add('visible');

    // Automatically close the welcome alert after 3 seconds
    setTimeout(() => {
        welcomeAlert.classList.remove('visible');
    }, 3000);
};
